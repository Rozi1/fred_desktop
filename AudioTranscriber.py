...
class AudioTranscriber:
    ...
    def transcribe_audio_queue(self, audio_queue,stop_flag):
        while True:
            if stop_flag.is_set():
                time.sleep(1)  # Sleep for a while if the stop flag is set
                continue
            ...
            try:
                ...
                sentence = self.audio_model.get_transcription(path)
            ...
            if text != '' and text.lower() != 'you':
                ...
                print(f"'{sentiment}'")  # Debug print
                sentiment = sentiment.strip().lower()

                if sentiment == "neutral":
                    sentiment+= ' 😐'
                    print('😐')  # Neutral face emoji
                elif sentiment == "positive":
                    print('😀')
                    sentiment+= ' 😀'   # Grinning face emoji
                elif sentiment == "negative":
                    sentiment+= ' 😞'
                    print('😞')  # Disappointed face emoji
                else:
                    print('Unknown sentiment')

                # time_spoken_str = time_spoken.strftime("%H:%M:%S")
                #  + "\n "+ time_spoken_str
                self.update_transcript(who_spoke, text+", "+sentiment, time_spoken)
